...
abstract class TaskManagerTest<T extends TaskManager>  {
    ...
    public void shouldAssignEpicsTimeCorrectly() {
        ...
        assertEquals(epic.getEndTime(), sb1.getEndTime(), "Epic with one subtask has different end time from its subtask");
        assertEquals(epic.getStartTime(), sb1.getStartTime(), "Epic with one subtask has different start time from its subtask");
    ...
    public void shouldNotCreateTasksWithOverlappingTimes() {
        LocalDateTime startTime = LocalDateTime.now();
        Duration duration = Duration.ofMinutes(30);
        Task task1 = new Task("Task", "task", TaskStatus.NEW, startTime, duration);
        manager.createTask(task1);
        Task task2 = new Task("Task", "task", TaskStatus.NEW, startTime.plusMinutes(31), duration);
        assertDoesNotThrow(() -> manager.createTask(task2),
                "Task with time interval from future is shown as overlapped");
        Task task3 = new Task("Task", "task", TaskStatus.NEW, startTime.minusMinutes(31), duration);
        assertDoesNotThrow(() -> manager.createTask(task3),
                "Task with time interval from past is shown as overlapped");
        Task task4 = new Task("Task", "task", TaskStatus.NEW, startTime.plusMinutes(29), duration);
        assertThrows(IllegalStateException.class, () -> manager.createTask(task4),
                "Task with overlapping interval is not throwing exception");
        Task task5 = new Task("Task", "task", TaskStatus.NEW, startTime.minusMinutes(29), duration);
    ...
    public void getPrioritizedTasks_returnsTasksSortedByStartTime() {
        ...
        manager.createEpic(epic);
        Task task1 = new Task("Task", "task", TaskStatus.NEW, startTime, duration);
        Subtask subtask  = new Subtask("Subtask", "task", TaskStatus.NEW, epic.getId(),  startTime.plusHours(1), duration);
        Task task2 = new Task("Task", "task", TaskStatus.NEW, startTime.plusHours(2), duration);
        manager.createTask(task1);
    ...
    public void shouldAssignEndTimeCorrectlyForTasksNSubtasks() {
        ...
        Task task = new Task("Task", "task", TaskStatus.NEW, startTime, duration);
    ...
    public void shouldUpdateEpicsStatusCorrectly() {
        ...
        Subtask subtask1 = new Subtask("Subtask", "subtask", TaskStatus.DONE, epicId);
        manager.createSubtask(subtask1);
        assertEquals(subtask1.getStatus(),
                epic.getStatus(), "Epic does not have the same status as its single subtask");

        Subtask subtask2 = new Subtask("Subtask", "subtask", TaskStatus.NEW, epicId);
        ...
        manager.updateSubtaskById(
                new Subtask(subtask2Id, "Subtask", "subtask", TaskStatus.DONE, epicId));
        assertEquals(TaskStatus.IN_PROGRESS,
    ...
    public void epicShouldSaveListOfSubtasksAfterUpdate() {
        Epic oldEpic = new Epic("Epic", "epic");
        manager.createEpic(oldEpic);
        int epicId = oldEpic.getId();
        Subtask subtask1 = new Subtask("Subtask", "subtask", TaskStatus.NEW, epicId);
        Subtask subtask2 = new Subtask("Subtask", "subtask", TaskStatus.NEW, epicId);
        manager.createSubtask(subtask1);
        manager.createSubtask(subtask2);
        Epic newEpic = new Epic(epicId, "Epic", "epic");
    ...
    public void shouldDeleteAllSubtasksIfAllEpicsDeleted() {
        ...
        Subtask subtask1 = new Subtask("Subtask", "subtask", TaskStatus.NEW, epic1Id);
        Subtask subtask2 = new Subtask("Subtask", "subtask", TaskStatus.NEW, epic2Id);
        manager.createSubtask(subtask1);
        manager.createSubtask(subtask2);
    ...
    public void shouldDeleteAllTasks() {
        Task task1 = new Task("Task", "task", TaskStatus.NEW);
        Task task2 = new Task("Task", "task", TaskStatus.NEW);
        manager.createTask(task1);
        manager.createTask(task2);
    ...
    public void shouldUpdateTaskCorrectly() {
        Task oldTask = new Task("Task", "task", TaskStatus.NEW);
        manager.createTask(oldTask);
        int oldTaskId = oldTask.getId();
        Task newTask = new Task(oldTaskId,"Task", "task", TaskStatus.NEW);
    ...
    public void shouldUpdateEpicCorrectly() {
        ...
        Epic newEpic = new Epic(oldEpicId,"Epic", "epic");
        manager.updateEpicById(newEpic);
    ...
    public void shouldUpdateSubtaskCorrectly() {
        ...
        manager.createEpic(epic);
        int epicId = epic.getId();
        Subtask oldSubtask = new Subtask("Subtask", "subtask", TaskStatus.NEW, epicId);
        manager.createSubtask(oldSubtask);
        int oldSubtaskId = oldSubtask.getId();
        Subtask newSubtask = new Subtask(oldSubtaskId,"Subtask", "subtask", TaskStatus.NEW, epicId);
